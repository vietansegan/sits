 ...
 * @author vietan
 */
public class AuthorShiftBackgroundSampler extends AbstractTopicShiftSampler {
    ...
    private int[] l;
    private int[][] z; // topic assignment for each word
    private int[][] x; // foreground/background topic switch for each word
    ...
    private FiniteMultinomial[] turn_switch;
    ...
    public String getSamplerName() {
        return this.getPrefix()
                ...
                + "_d-" + formatter.format(this.hyperparams.get(DELTA))
                + "_K-" + K
                + "_opt-" + paramOptimized;
    }
    ...
    public void configure(String folder, int[][][] words, int[][] authors,
        ...
        configure(folder, newWords, newSpeakers, K, J, V, alpha, beta, gamma, delta,
    ...
    protected void initialize() {
        ...
        topic_word = new SymmetricFiniteMultinomials(K, V, this.hyperparams.get(BETA));
        author_shift = new SymmetricFiniteMultinomials(J, 2, this.hyperparams.get(GAMMA));
        turn_switch = new FiniteMultinomial[T];
        ...
        for (int t = 0; t < T; t++) {
            ...
            turn_switch[t] = new FiniteMultinomial(2, this.hyperparams.get(DELTA) * 2, 0.5);
            ...
            for (int n = 0; n < Nt; n++) {
                x[t][n] = rand.nextInt(2);

                if (x[t][n] == FOREGROUND) // choose a foreground topic
                {
...
//                    System.out.println("iter = " + iter + ". sampledL size = " + sampledL.length);
                    ...
                    for (int i = 0; i < sampledL.length; i++) {
                        sampledL[i] = l[i];
                    }
                    ...
                    if (paramOptimized) {
                        if (verbose) {
                            System.out.print(". Slice sampling ...");
                        }
    ...
    private void sampleZ(int t, int n) {
        ...
        int cur_segment = segment_index[t];
        int cur_switch = x[t][n];

        topic_word.decrement(cur_topic, cur_word);
        turn_switch[t].decrement(cur_switch);
